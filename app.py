...
def get_account_credentials(region: str) -> str:
    r = region.upper()
    if r == "IND":
        return "uid=3968051215&password=919F15883BBC2FB4FBDFE80B85E655B7882821D78B93A0D0745A4BB9F48ECFE0"
...
def decode_unicode(value):
   # تحويل النصوص المشفرة بصيغة \uXXXX إلى نصوص مقروءة
...
def format_response(data):
    return {
        "AccountInfo": {
            ...
            "AccountName": decode_unicode(data.get("basicInfo", {}).get("nickname")),
        ...
        "GuildInfo": {
            ...
            "GuildName": decode_unicode(data.get("clanBasicInfo", {}).get("clanName")),
        ...
        "captainBasicInfo": {
            ...
            "nickname": decode_unicode(data.get("captainBasicInfo", {}).get("nickname")),
        ...
        "petInfo": {
            **data.get("petInfo", {}),
            "name": decode_unicode(data.get("petInfo", {}).get("name"))
        },
        "socialinfo": {
            "AccountLanguage": data.get("socialInfo", {}).get("language"),
            "AccountPreferMode": data.get("socialInfo", {}).get("modePrefer"),
            "AccountSignature": decode_unicode(data.get("socialInfo", {}).get("signature"))
        }
    }
...
if __name__ == '__main__':
    asyncio.run(startup())
    app.run(host='0.0.0.0', port=5000, debug=True)  # أو 5020 حسب اختيارك